...
public:
    ...
    bool WriteZNSSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZNSSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZNSSeed_deprecated(uint256& seed);
    bool EraseZNSSeed();
    bool EraseZNSSeed_deprecated();

    bool WriteZNSCount(const uint32_t& nCount);
    bool ReadZNSCount(uint32_t& nCount);
