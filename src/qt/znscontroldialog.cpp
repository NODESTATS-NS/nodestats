...
std::set<std::string> ZNSControlDialog::setSelectedMints;
std::set<CMintMeta> ZNSControlDialog::setMints;

ZNSControlDialog::ZNSControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZNSControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZNSControlDialog::~ZNSControlDialog()
{
    delete ui;
}

void ZNSControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZNSControlDialog::updateList()
{
...
void ZNSControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZNSControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZNS_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZNSControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZNSControlDialog::GetSelectedMints()
{
...
void ZNSControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
