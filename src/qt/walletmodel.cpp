...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered nodestats address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 NS.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal nodestats:URI (nodestats:XyZ...?message=example)
            {
...
static void NotifyznsReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyznsReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyznsReset.connect(boost::bind(NotifyznsReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyznsReset.disconnect(boost::bind(NotifyznsReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->znsTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
