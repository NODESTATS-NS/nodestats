...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::NS);
    ...
    if (!settings.contains("nAnonymizeNodestatsAmount"))
        settings.setValue("nAnonymizeNodestatsAmount", 1000);

    nAnonymizeNodestatsAmount = settings.value("nAnonymizeNodestatsAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeNodestatsAmount"))
        SoftSetArg("-anonymizenodestatsamount", settings.value("nAnonymizeNodestatsAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in nodestats.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeNodestatsAmount:
            return QVariant(nAnonymizeNodestatsAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeNodestatsAmount:
            nAnonymizeNodestatsAmount = value.toInt();
            settings.setValue("nAnonymizeNodestatsAmount", nAnonymizeNodestatsAmount);
            emit anonymizeNodestatsAmountChanged(nAnonymizeNodestatsAmount);
            break;
        case CoinControlFeatures:
