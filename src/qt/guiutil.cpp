...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Nodestats address (e.g. %1)").arg("C6M1kT6PQaieDqNv2ajcTJtcfip2krta6a"));
...
{
    // return if URI is not valid or is no Nodestats: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::NS, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert nodestats:// to nodestats:
    //
    //    Cannot handle this later, because nodestats:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::NS, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open nodestats.conf with the associated application */
...
int TableViewLastColumnResizingFixer::getColunsWidth()
{
    int nColunsWidthSum = 0;
    for (int i = 0; i < columnCount; i++) {
        nColunsWidthSum += tableView->horizontalHeader()->sectionSize(i);
    }
    return nColunsWidthSum;
}
...
int TableViewLastColumnResizingFixer::getAvailableWidthForColumn(int column)
{
    ...
    if (nTableWidth > 0) {
        int nOtherColsWidth = getColunsWidth() - tableView->horizontalHeader()->sectionSize(column);
...
// Make sure we don't make the coluns wider than the tables viewport width.
void TableViewLastColumnResizingFixer::adjustTableColunsWidth()
{
...
{
    if ((getColunsWidth() - this->tableView->horizontalHeader()->width()) != 0) {
        disconnectViewHeadersSignals();
        resizeColumn(secondToLastColumnIndex, getAvailableWidthForColumn(secondToLastColumnIndex));
        connectViewHeadersSignals();
    }
 ...
 * the resize modes of the last 2 coluns of the table and
 */
TableViewLastColumnResizingFixer::TableViewLastColumnResizingFixer(QTableView* table, int lastColMinimumWidth, int allColsMinimumWidth) : tableView(table),
                                                                                                                                          ...
                                                                                                                                          allColunsMinimumWidth(allColsMinimumWidth)
{
    columnCount = tableView->horizontalHeader()->count();
    lastColumnIndex = columnCount - 1;
    secondToLastColumnIndex = columnCount - 2;
    tableView->horizontalHeader()->setMinimumSectionSize(allColunsMinimumWidth);
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Nodestats.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Nodestats.lnk
...
{
    return GetAutostartDir() / "nodestats.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a nodestats.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Nodestats\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the nodestats app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add nodestats app to startup item list
