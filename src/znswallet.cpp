...
CznsWallet::CznsWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zns seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZNSSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZNSSeed_deprecated()) {
                    LogPrintf("%s: Updated zns seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zns seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zns wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CznsWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZNSCount(nCountLastUsed);
    else if (!walletdb.ReadZNSCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CznsWallet::Lock()
{
    seedMaster = 0;
}

void CznsWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CznsWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZNS(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CznsWallet::LoadMintPoolFromDB()
{
...
void CznsWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CznsWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CznsWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->znsTracker->HasPubcoinHash(pMint.first)) {
...
bool CznsWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZNS(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to znsTracker which also adds to database
    pwalletMain->znsTracker->Add(dMint, true);
...
void CznsWallet::SeedToZNS(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CznsWallet::GetZerocoinSeed(uint32_t n)
{
...
void CznsWallet::GenerateDeterministicZNS(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CznsWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CznsWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
