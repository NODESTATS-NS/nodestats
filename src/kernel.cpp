...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZNSStake(spend));
    } else {
        ...
        CNSStake* NSInput = new CNSStake();
        NSInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(NSInput);
    }
