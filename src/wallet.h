...
enum AvailableCoinsType {
    ...
    EXCEPT_DENOMINATED_OR_MASTERNODE_COLLATERAL = 4, // ONLY_NONDENOMINATED and not 15000 NS at the same time
    ONLY_MASTERNODE_COLLATERAL = 5,       // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zns send
enum ZerocoinSpendStatus {
    ZNS_SPEND_OKAY = 0,                            // No error
    ZNS_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZNS_WALLET_LOCKED = 2,                         // Wallet was locked
    ZNS_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZNS_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZNS_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZNS_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZNS_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZNS_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZNS_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZNS_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZNS_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZNS_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZNS_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZNS_SPENT_USED_ZNS = 14,                      // Coin has already been spend
    ZNS_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZNS_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZNSBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZNSOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fznsAuto) const;
    ...
    */
    ...
    CznsWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CznsTracker> znsTracker;
    ...
    void setZWallet(CznsWallet* zwallet)
    {
        zwalletMain = zwallet;
        znsTracker = std::unique_ptr<CznsTracker>(new CznsTracker(strWalletFile));
    }

    CznsWallet* getZWallet() { return zwalletMain; }
    ...
    /** zns reset */
    boost::signals2::signal<void()> NotifyznsReset;
