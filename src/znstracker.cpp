...
CznsTracker::CznsTracker(std::string strWalletFile)
{
...
CznsTracker::~CznsTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CznsTracker::Init()
{
...
bool CznsTracker::Archive(CMintMeta& meta)
{
...
bool CznsTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CznsTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CznsTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CznsTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CznsTracker::GetSerialHashes()
{
...
CAmount CznsTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CznsTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CznsTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CznsTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CznsTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CznsTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CznsTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CznsTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CznsTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CznsTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CznsTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CznsTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CznsTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CznsTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CznsTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CznsTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CznsTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzns from DB\n", __func__, listDeterministicDB.size());
    }
