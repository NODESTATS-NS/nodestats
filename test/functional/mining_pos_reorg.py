...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected NS and zNS supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zNSsupply']:
                assert_equal(node['zNSsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check NS and zNS supply at the beginning
        # ------------------------------------------
        # zNS supply: 2 coins for each denomination
        ...
        # NS supply: block rewards minus burned fees for minting
        ...
        # Verify that NS and zNS supplies were properly updated after the spends and reorgs
        self.log.info("Check NS and zNS supply...")
